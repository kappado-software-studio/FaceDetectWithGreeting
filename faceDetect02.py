...
for line in f:
    ...
    l = line.split()
        # データを読み込んで28x28に縮小
        #img_src = cv2.imread(l[0])
    #img_filename = cv2.imread(line)
    print('filename=%s' % line)
# 集めた画像データから顔が検知されたら、切り取り、保存する。
    img_filename = './'+keyword+'/'+line
    print ('img_file:'+img_filename)
    if not os.path.exists(img_filename):
        print (img_filename+':'+'NotExist')
        continue
